...
def init(C):
    ...
    try:
        ...
        print(f"[{current_time}] 创业板ETF MACD策略启动 | 单笔金额: {A.amount}元")
...
def get_reliable_price(C):
    ...
    # 1. 尝试获取1分钟收盘价
    try:
        min_data = C.get_market_data(['close'], [A.stock], '1m', 1)
        if min_data and A.stock in min_data and not min_data[A.stock].empty:
            last_close = min_data[A.stock].iloc[0]['close']
            if last_close > 0:
    ...
    # 2. 尝试日线收盘价
    try:
        day_data = C.get_market_data(['close'], [A.stock], '1d', 1)
        if day_data and A.stock in day_data and not day_data[A.stock].empty:
            last_close = day_data[A.stock].iloc[0]['close']
            if last_close > 0:
    ...
    # 3. 尝试获取tick价格
    try:
        tick_data = C.get_market_data(['quoter'], [A.stock], 'tick', 1)
        if tick_data and A.stock in tick_data and 'lastPrice' in tick_data[A.stock]:
            last_price = tick_data[A.stock]['lastPrice'][0]
            if last_price > 0:
...
def is_macd_above_zero(C):
    """检查30分钟MACD是否在零轴上方"""
    try:
        data = C.get_market_data(['close'], [A.stock], '30m', 50, dividend_type='front_ratio')
        if data and A.stock in data:
            closes = data[A.stock]
            if len(closes) < max(A.macd_fast, A.macd_slow):
                return False
...
def is_macd_divergence(C):
    """检测5分钟MACD底背离"""
    try:
        data = C.get_market_data(['close'], [A.stock], '5m', 40, dividend_type='front_ratio')
        if data and A.stock in data:
            closes = data[A.stock]
            closes = list(closes)  # 保证为list，防止是DataFrame或Series
            if len(closes) < 20:
                return False
